...
public abstract class ProtobufferTestAbstract<T extends Declaration,S extends DeclarationBinder> {
    ...
    @Mock
    protected ServiceReference fuchsiaDeclarationBinderServiceRegistration;

    protected AddressBookProtos.AddressBookService protobufferRemoteServiceProxy;
    ...
    public void initInterceptors() throws Exception{
        when(context.getServiceReference(PackageAdmin.class.getName())).thenReturn(fuchsiaDeclarationBinderServiceRegistration);
        when(packageAdminMock.getExportedPackage(protobufferRemoteService.getClass().getName())).thenReturn(importPackageForClass);
        when(context.getService(fuchsiaDeclarationBinderServiceRegistration)).thenReturn(packageAdminMock);
        when(context.getBundle()).thenReturn(bundeToLoadClassFrom);
        when(fuchsiaDeclarationBinderServiceRegistration.getProperty(org.osgi.framework.Constants.SERVICE_ID)).thenReturn(1l);
